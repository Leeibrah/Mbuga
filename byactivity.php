...
<body>
		...
		$sql = "select p.id, p.park_name, p.park_location, count(r.park_id) reportCount " . 
				"from parks p left join parks r on r.park_id = p.park_id " .
				"group by p.park_id order by p.park_name where activity = ".$activity;
		//$sqll = "SELECT * FROM sites";
		$sqll = "select p.id, p.park_name, p.park_location, count(r.id) reportCount " . 
